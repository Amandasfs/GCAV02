...
@arquivos_bp.route('/arquivos/<int:NB>', methods=['PUT'])
@requer_autenticacao
def atualizar_arquivo(NB):
    try:
        data = request.get_json()
        atualizacao = {k: v for k, v in data.items() if k != 'NB'}
        
        resultado_mongo = mongo_handler.atualizar_arquivo(NB, atualizacao)
        resultado_json = json_handler.atualizar_arquivo(NB, atualizacao)
...
@arquivos_bp.route('/arquivos/<int:NB>', methods=['DELETE'])
@requer_autenticacao
def deletar_arquivo(NB):
    try:
        resultado_mongo = mongo_handler.deletar_arquivo(NB)
        resultado_json = json_handler.deletar_arquivo(NB)
        
        if resultado_mongo.deleted_count > 0 or resultado_json:
