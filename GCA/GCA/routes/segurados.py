...
@segurados_bp.route('/segurados/<int:cpf>', methods=['DELETE'])
@requer_autenticacao
def deletar_segurado(cpf):
    try:
        resultado_mongo = mongo_handler.deletar_segurado(cpf)
        resultado_json = json_handler.deletar_segurado(cpf)
        
        if resultado_mongo.deleted_count > 0 or resultado_json:
            return jsonify({"mensagem": "Segurado deletado com sucesso"}), 200
        return jsonify({"erro": "Segurado não encontrado"}), 404
    except Exception as e:
        return jsonify({"erro": str(e)}), 500