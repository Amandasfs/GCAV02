...
@caixas_bp.route('/caixas/<int:codigo>', methods=['DELETE'])
@requer_autenticacao
def deletar_caixa(codigo):
    try:
        resultado_mongo = mongo_handler.deletar_caixa(codigo)
        resultado_json = json_handler.deletar_caixa(codigo)
        
        if resultado_mongo.deleted_count > 0 or resultado_json:
            return jsonify({"mensagem": "Caixa deletada com sucesso"}), 200
        return jsonify({"erro": "Caixa não encontrada"}), 404
    except Exception as e:
        return jsonify({"erro": str(e)}), 500